...
2. As a 用户, I want 看到总资产较**最近一次快照**（手动或自动，标注其年月日分时）的涨跌额与涨跌方向；无快照基线时显示「—」, so that 我快速感知资产变化
...
20. As a 用户, I want DEX 账户展开显示逐 token 明细（代币、数量、价格、USD 价值，按价值排序，< $1 与风险代币不展示；**明细在展开时惰性拉取，刷新只取总值**）, so that 我知道链上资产构成
...
- **统一计价**：共享计价模块——Binance 批量 ticker（浏览器直连）+ OKX v5 行情兜底；DEX 账户用 OKX 返回的 tokenPrice。**无跨刷新价格缓存**（每次刷新取当刻数据）；单次刷新内计价全并发 + 按资产 in-flight 去重（同一刷新内同一资产只取一次价），每跳带 6s 超时。
- **环境变量**（Vercel env / `.env.local`，git 忽略）：`OKX_DEX_API_KEY` / `OKX_DEX_SECRET` / `OKX_DEX_PASSPHRASE`。

### 刷新与快照

- 刷新：手动 + 每日 UTC+8 24:00 自动（页面打开时）。并发调用各平台，单账户失败隔离（保留上次值 + 错误标记 + 可读错误映射：-2015 权限、429 限流、超时）；**每账户 10s 超时**（卡死端点不拖累整次刷新）。手动刷新**永不写快照**（纯实时查看）；刷新采用「一次拿全再更新」，不逐账户上屏。
- 快照：仅**整点自动**（24:00 刷新后落点）+ **手动按钮**（可保留，同日后者覆盖前者）；日期按 UTC+8 日历日，同一天去重；每账户粒度存储，图表画总资产。缺拍不补（页面夜里没开 → 该日空缺）。
- 「较上次」对比：基准 = **最近一次快照（手动或自动）**，即序列中「记录时刻早于当前余额时刻（lastRefreshed）」的最近点；无基线 → 显示「—」。快照记录时刻存 `at`（旧导出无 `at` 视为可用基线）；UI 标注基线快照时间（UTC+8 年月日分时）。
