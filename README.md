...
Get the **three-part key** (API Key + Secret Key + Passphrase) from the **OKX OnchainOS dev portal** → **https://web3.okx.com/onchainos/dev-portal**, then fill them in.

> ⚠️ Wrap the passphrase in double quotes if it contains `#` / `!`, otherwise it is truncated (OKX error 50105).

Other exchanges — no setup needed: just enter each exchange's API key in the "Add account" form (create them on each exchange's site; read-only permission recommended).
